package io.github.angelofcreation.hierophanticsplus.config
...
object HierophanticsPlusClientConfig {
    @JvmStatic
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HierophanticsPlusServerConfig.holder.config = HierophanticsPlusServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HierophanticsPlusServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = HierophanticsPlus.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HierophanticsPlusServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HierophanticsPlusServerConfig.ServerConfig()
    }
