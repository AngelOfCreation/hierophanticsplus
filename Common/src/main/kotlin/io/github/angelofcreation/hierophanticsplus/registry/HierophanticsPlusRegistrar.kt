...
typealias RegistrarEntry<T> = HierophanticsPlusRegistrar<T>.Entry<out T>

abstract class HierophanticsPlusRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(HierophanticsPlus.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HierophanticsPlusRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
